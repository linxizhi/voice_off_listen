...
const char * ASR_RES_PATH        = "fo|/home/linxi/catkin_ws/src/asr_offline_record_sample/bin/msc/res/asr/common.jet"; //离线语法识别资源路径
const char * GRM_BUILD_PATH      = "/home/linxi/catkin_ws/src/asr_offline_record_sample/bin/msc/res/asr/GrmBuilld"; //构建离线语法识别网络生成数据保存路径
const char * GRM_FILE            = "/home/linxi/catkin_ws/src/asr_offline_record_sample/bin/msc/res/asr/call1.bnf"; //构建离线识别语法网络所用的语法文件
...
static void show_result(char *string, char is_over)
{
	...
	f_result=string;
	/*ros::NodeHandle nh;
	ros::Publisher pub2=nh.advertise<std_msgs::String>("trans",10);
	if (ros::ok())
	{
		std_msgs::String msg;
		msg.data=f_result;
...
static void demo_mic(const char* session_begin_params)
{
	...
	while(i++ < 20)
	{
		//if (iat.session_id==NULL)
		//	break;
		if (iat.session_id==NULL || iat.recorder==NULL)
			break;
		sleep(1);	
	}	
	errcode = sr_stop_listening(&iat);
...
{	
	const char *login_config    = "appid = 646baf12"; //登录参数
	UserData asr_data; 
	int ret                     = 0 ;
	char c;
	...
	ros::Publisher pub1=nh.advertise<std_msgs::String>("trans",10);
